 ...
 * ============================================================
 *  Unit-тест — это маленький кусок кода, который проверяет
 *  одну конкретную функцию: "я передаю вот такие данные →
 *  жду вот такой результат". Если результат не совпал — тест
 *  падает и сразу видно, что сломалось.
 *
 *  Мы не используем сторонние библиотеки — всё написано
 *  на чистом JS. Открой index.html в браузере, потом открой
 *  консоль (F12 → Console) и увидишь результаты тестов.
 * ============================================================
 */
...
/**
 * Проверяет условие и выводит результат в консоль
 * @param {string} description - что мы проверяем (читаемое описание)
 * @param {boolean} condition  - должно быть true, если всё хорошо
 */
...
/** Группирует тесты под одним названием */
...
/*
 * Мы тестируем чистые функции (те, что не трогают DOM).
 * Они скопированы из app.js, чтобы тест мог их вызывать
 * без браузера (или до загрузки страницы).
 */
